      ...
      <div className="mt-6 space-y-3">{meals.map((meal) => {
        ...
        return <div key={meal.id} className={`overflow-hidden rounded-2xl border ${dayCompleted ? "border-[#b6c77b] bg-[#eef5d2]" : "border-[#d3dbcf] bg-white/50"}`}>
          <button type="button" onClick={() => setOpenMeal(isOpen ? null : meal.id)} className="flex w-full items-center justify-between gap-3 p-4 text-left"><span><span className="block text-xs font-semibold uppercase tracking-[0.14em] text-[#819078]">{meal.suggestedTime ? meal.suggestedTime.slice(0, 5) : "Flexible"}</span><span className="mt-1 flex items-center gap-2 text-lg font-semibold">{isOutside && <MapPin size={16} className="text-[#72873f]" />}{displayedMealName}</span></span><span className="flex items-center gap-3 text-sm text-[#68736b]"><span>{meal.targetCalories} kcal</span><ChevronDown size={18} className={isOpen ? "rotate-180" : ""} /></span></button>
            {isOpen && <div className="border-t border-[#d3dbcf] px-4 pb-5 pt-3"><div className="space-y-2">{meal.items.map((item) => { const selectedFood = item.alternativeGroup ? selectedAlternatives[`${meal.id}:${item.alternativeGroup}`] : undefined; const displayedName = selectedFood ?? item.name; const displayedQuantity = selectedFood === item.selectedFoodName ? item.selectedQuantityGrams ?? item.quantityGrams : equivalentQuantity(item, selectedFood); const style = foodTypeStyle(item.role); return <div key={item.id} className={`flex items-center justify-between gap-4 rounded-xl border px-4 py-3 ${style.className}`}><div><div className="flex flex-wrap items-center gap-2"><p className="font-medium text-[#18231f]">{displayedName}</p><span className="rounded-full bg-white/70 px-2 py-0.5 text-[10px] font-semibold">{style.label}</span></div><p className="mt-1 text-xs opacity-75">{item.alternativeGroup ? "Puedes cambiarlo por una equivalencia" : "Parte de tu propuesta"}</p></div><span className="whitespace-nowrap text-sm font-semibold text-[#60703d]">{displayedQuantity} g · cocinado</span></div>; })}</div><div className="mt-4 flex gap-2 border-l-2 border-[#b6c77b] pl-3 text-sm text-[#68736b]"><ChefHat className="shrink-0 text-[#72873f]" size={17} /><p>{preparationForMeal({ ...meal, name: displayedMealName })}</p></div>{Object.entries(alternatives).filter(([group]) => meal.items.some((item) => item.alternativeGroup === group)).map(([group, foods]) => <div key={group} className="mt-5"><div className="flex items-center gap-2 text-sm font-semibold"><RefreshCw size={15} className="text-[#72873f]" /> Cambiar {group === "carb_base" ? "hidrato" : group === "protein" ? "proteína" : group === "vegetable" ? "vegetal" : "grasa"}</div><div className="mt-3 flex flex-wrap gap-2">{foods.map((food) => <button key={food} type="button" onClick={() => selectAlternative(meal.id, group, food, meal.items)} className={`rounded-full border px-3 py-2 text-xs font-medium ${selectedAlternatives[`${meal.id}:${group}`] === food ? "border-[#72873f] bg-[#e7f5b4] text-[#60703d]" : "border-[#cfd7c8] text-[#68736b] hover:border-[#72873f]"}`}>{food}</button>)}</div><p className="mt-2 text-xs text-[#819078]">La cantidad se recalcula para mantener la equivalencia.</p></div>)}</div>}
  </div>;
      })}</div>
    </section>
  );
